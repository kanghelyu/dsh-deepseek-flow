/* deepseek-flow client-rev:49c37822c3e7 */
...
function normalEdges(edges = []) {
  return edges.filter((edge) => !isFeedbackEdge(edge));
}
function compareById(left, right) {
  return String(left).localeCompare(String(right));
}
function stableTopologicalOrder(nodes = [], edges = [], compare = compareById) {
...
function topologyEdge(edge) {
  return definedEntries({
    ...
    feedback: edge?.feedback === void 0 ? void 0 : {
...
function flowToCanvasNodes(flow, positionOverrides) {
  return (flow?.nodes ?? []).map((node) => ({
    ...
    data: {
      ...node.data,
      kind: node.kind,
      ...node.kind === "condition" ? { gateType: conditionGateType(node, (flow?.edges ?? []).filter((edge) => edge.source === node.id && !isFeedbackEdge(edge))) } : {},
...
function connectionProblem(nodes, edges, connection, branch = null) {
  ...
  const feedback = connection.feedback !== void 0;
...
function reconnectFlowEdge(oldEdge, connection, edges) {
  return edges.map((edge) => edge.id === oldEdge.id ? {
    ...
    ...isFeedbackEdge(edge) ? { sourceHandle: null, targetHandle: null } : { sourceHandle: connection.sourceHandle ?? null, targetHandle: connection.targetHandle ?? null }
  } : edge);
}
...
function graphEdgeGeometry(edge, byId, lane = 0) {
  ...
  const bend = reverse ? Math.max(108, Math.abs(end.x - start.x) * 0.56) + lane * 28 : Math.max(54, Math.abs(end.x - start.x) * 0.46) + lane * 16;
  ...
  return {
    ...
    path: reverse ? `M ${start.x} ${start.y} C ${start.x + bend} ${start.y + lift}, ${end.x - bend} ${end.y + lift}, ${end.x} ${end.y}` : `M ${start.x} ${start.y} C ${start.x + bend} ${start.y}, ${end.x - bend} ${end.y}, ${end.x} ${end.y}`
  };
}
function GraphCanvas({
  ...
  const [liveDrag, setLiveDrag] = (0, import_react.useState)(null);
  const [connectionDraft, setConnectionDraft] = (0, import_react.useState)(null);
  const renderNodes = (0, import_react.useMemo)(() => liveDrag ? nodes.map((node) => node.id === liveDrag.id ? { ...node, position: liveDrag.position } : node) : nodes, [nodes, liveDrag]);
  const nodesRef = import_react.default.useRef(renderNodes);
  (0, import_react.useEffect)(() => {
    nodesRef.current = renderNodes;
  }, [renderNodes]);
  const byId = (0, import_react.useMemo)(() => new Map(renderNodes.map((node) => [node.id, node])), [renderNodes]);
  ...
  const fitView = (0, import_react.useCallback)((options = {}) => {
    ...
    if (!rect || nodes2.length === 0) return;
    const requestedIds = new Set((options.nodes ?? []).map((node) => typeof node === "string" ? node : node?.id).filter(Boolean));
    const visibleNodes = requestedIds.size > 0 ? nodes2.filter((node) => requestedIds.has(node.id)) : nodes2;
  ...
  const screenCenter = (0, import_react.useCallback)(() => {
    const rect = rootRef.current?.getBoundingClientRect();
  ...
  for (const edge of edges) {
    ...
    const displayLabel = isFeedbackEdge(edge) ? `${copy.feedbackEdgeLabel} ${edge.feedback?.maxIterations ?? "?"}` : edge.autoLogicLabel ? branchDisplayLabel(gateBranchForEdge(edge), copy) : edge.label;
    edgeElements.push(
  ...
  return import_react.default.createElement(
    ...
    import_react.default.createElement(
      ...
      renderNodes.map((node) => import_react.default.createElement(
        "div",
        {
          key: node.id,
...
function text(language) {
  return language === "zh" ? {
    ...
    shared: "\u5171\u4EAB",
    flowSwitcherTitle: "\u5207\u6362\u5DE5\u4F5C\u6D41",
    flowSwitcherEmpty: "\u6682\u65E0\u5DE5\u4F5C\u6D41",
    flowSwitcherPlaceholder: "\u9009\u62E9\u5DE5\u4F5C\u6D41\u2026",
    flowGroupCurrent: "\u5F53\u524D Session",
    flowGroupShared: "\u5171\u4EAB\u6A21\u677F",
    flowGroupHistory: "\u5176\u4ED6 Session",
    noFlowWithHistory: "\u672C Session \u8FD8\u6CA1\u6709\u5DE5\u4F5C\u6D41",
    emptyFlowHintWithHistory: "\u4ECE\u4E0A\u65B9\u4E0B\u62C9\u83DC\u5355\u9009\u62E9\u4E00\u4E2A\u5386\u53F2\u5DE5\u4F5C\u6D41\uFF0C\u70B9\u51FB\u201C\u5BFC\u5165 JSON\u201D\uFF0C\u6216\u56DE\u5230 Session \u8BA9 Agent \u7528 flow_create \u65B0\u5EFA",
    flowNotFound: "\u672A\u627E\u5230\u8BE5\u5DE5\u4F5C\u6D41",
    importLabel: "\u5BFC\u5165 JSON",
    exportLabel: "\u5BFC\u51FA JSON",
    save: "\u4FDD\u5B58",
    topologyApply: "\u5E94\u7528\u4FEE\u6539",
    ...
    shared: "Shared",
    flowSwitcherTitle: "Switch workflow",
    flowSwitcherEmpty: "No workflow yet",
    flowSwitcherPlaceholder: "Choose a workflow\u2026",
    flowGroupCurrent: "Current session",
    flowGroupShared: "Shared templates",
    flowGroupHistory: "Other sessions",
    noFlowWithHistory: "No workflow in this session yet",
    emptyFlowHintWithHistory: "Pick a past workflow from the dropdown above, use Import JSON, or return to the Session and let the Agent create one with flow_create",
    flowNotFound: "Workflow not found",
...
function Studio({ connection, sessionId, language }) {
  ...
  const [allFlows, setAllFlows] = (0, import_react2.useState)([]);
  ...
  const [feedbackConnectionMode, setFeedbackConnectionMode] = (0, import_react2.useState)(false);
  ...
  const [assistProvider, setAssistProvider] = (0, import_react2.useState)(null);
  ...
  const pollsRef = import_react2.default.useRef(/* @__PURE__ */ new Set());
  const lastDraftPayloadRef = import_react2.default.useRef(null);
  ...
  const [deleteFlowConfirm, setDeleteFlowConfirm] = (0, import_react2.useState)(false);
  const [deleteFlowBusy, setDeleteFlowBusy] = (0, import_react2.useState)(false);
  const [switchFlowTarget, setSwitchFlowTarget] = (0, import_react2.useState)(null);
  const [docPathDraft, setDocPathDraft] = (0, import_react2.useState)(null);
  ...
  const assistMenuRef = import_react2.default.useRef(null);
  const flowMetaSnapshotRef = import_react2.default.useRef(null);
  const initialLoadRef = import_react2.default.useRef(null);
  const validationStoreRef = import_react2.default.useRef(/* @__PURE__ */ new Map());
  const draftTimerRef = import_react2.default.useRef(null);
  const draftSavedRef = import_react2.default.useRef(false);
  const flushDocumentsRef = import_react2.default.useRef(null);
  const pendingDocumentSnapshotRef = import_react2.default.useRef(null);
  const flowMetaSnapshot = (0, import_react2.useCallback)((items) => JSON.stringify(
  ...
  const restorePersistedDraft = (0, import_react2.useCallback)(async (flow) => {
    if (!flow?.id) return;
    try {
  ...
  (0, import_react2.useEffect)(() => {
    if (!assistMenuOpen) return void 0;
  ...
  (0, import_react2.useEffect)(() => keepLayout("deepseek-flow:assistant-instruction", assistantInstruction), [assistantInstruction]);
  (0, import_react2.useEffect)(() => () => {
  ...
  const currentFlow = (0, import_react2.useMemo)(() => flows.find((f) => f.id === currentId) ?? null, [currentId, flows]);
  const currentDraftFlow = (0, import_react2.useMemo)(
    () => currentFlow ? serializeFlow(currentFlow, nodes, edges) : null,
    [currentFlow, nodes, edges]
  );
  const currentTopologySignature = (0, import_react2.useMemo)(
  ...
  const topologyDelta = (0, import_react2.useMemo)(() => currentDraftFlow ? topologyDiff(persistedFlowRef.current ?? {
    ...currentDraftFlow,
    nodes: [],
    edges: [],
    inputs: [],
    outputs: []
  }, currentDraftFlow) : null, [currentDraftFlow, persistedTopologySignature]);
  const selectedNode = (0, import_react2.useMemo)(() => nodes.find((n) => n.id === selected) ?? null, [nodes, selected]);
  const selectedFlowEdge = (0, import_react2.useMemo)(() => edges.find((edge) => edge.id === selectedEdge) ?? null, [edges, selectedEdge]);
  ...
  const discardAndSwitchFlow = (0, import_react2.useCallback)(async () => {
  ...
  const selectedConditionInputs = selectedNode?.data?.kind === "condition" ? edges.filter((edge) => edge.target === selectedNode.id && !isFeedbackEdge(edge)).map((edge) => {
  ...
  const commitConnection = (0, import_react2.useCallback)((conn, requestedBranch = null) => {
    ...
    const edge = {
      ...
      type: "workflow",
      ...conn.feedback === void 0 ? {} : { feedback: conn.feedback },
      ...condition && conn.feedback === void 0 ? {
        sourceHandle: branch,
  ...
  const patchGateType = (gateType) => {
    ...
    if (edgesRef.current.some((edge) => edge.source === selectedNode.id && edge.feedback === void 0)) {
  ...
  const patchSelectedEdgeFeedback = (0, import_react2.useCallback)((patch) => {
    if (!selectedEdge) return;
    rememberGraph();
    markCanvasTopologyEdit();
  ...
  const clearSelectedEdgeFeedback = (0, import_react2.useCallback)(() => {
    if (!selectedEdge) return;
    rememberGraph();
    markCanvasTopologyEdit();
    const nextEdges = edgesRef.current.map((edge) => {
  ...
  (0, import_react2.useEffect)(() => {
    if (!currentId || !dirty && !topologyDirty || topologyApplyBusy) return void 0;
    if (draftTimerRef.current) clearTimeout(draftTimerRef.current);
  ...
  const pollAssist = (agentRequestId, onDone) => {
    ...
    const timer = setInterval(async () => {
      const expired = Date.now() - startedAt > 20 * 6e4;
      try {
  ...
  (0, import_react2.useEffect)(() => {
    const onKeyDown = (event) => {
      ...
      if (!editingText && event.key === "Escape") {
        ...
        if (workflowOptimizeConfirm) {
          setWorkflowOptimizeConfirm(false);
          return;
        }
        if (deleteFlowConfirm && !deleteFlowBusy) {
          setDeleteFlowConfirm(false);
          return;
        }
        if (switchFlowTarget) {
        ...
        if (topologyApplyConfirm && !topologyApplyBusy) {
          setTopologyApplyConfirm(false);
          return;
        }
        setSelected(null);
  ...
  const flowGroups = (0, import_react2.useMemo)(() => {
    const groups = /* @__PURE__ */ new Map();
    const keyOf = (entry) => entry.ownerSessionId === null ? "__shared__" : entry.ownerSessionId === sessionId ? "__current__" : entry.ownerSessionId;
    const order = [];
  ...
  const toolbar = import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        ...
        flowGroups.map((group) => import_react2.default.createElement(
          "optgroup",
          {
            key: group.key,
            label: group.key === "__current__" ? t.flowGroupCurrent : group.key === "__shared__" ? t.flowGroupShared : t.flowGroupHistory
          },
  ...
  const inspector = import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      activeDoc === "workflow" && currentFlow ? [
        ...
        import_react2.default.createElement("h3", { key: "title" }, t.edgeProperties),
        import_react2.default.createElement(
          "div",
          { key: "path", className: "df-pathbox" },
          import_react2.default.createElement("span", { className: "df-pathbox__label" }, t.edgeRoute),
          import_react2.default.createElement("span", { className: "df-pathbox__value" }, `${selectedFlowEdge.source} -> ${selectedFlowEdge.target}`)
        ),
        selectedFlowEdge.feedback ? import_react2.default.createElement(
          "div",
          { key: "feedback", className: "df-advanced__content" },
          import_react2.default.createElement("strong", null, t.feedbackLoop),
          import_react2.default.createElement("span", { style: { color: "var(--df-ink-2)", fontSize: 12 } }, t.feedbackLoopNote),
          import_react2.default.createElement(
            "label",
            null,
            t.feedbackIterations,
          ...
          selectedFlowEdge.source !== selectedFlowEdge.target && import_react2.default.createElement("button", {
            className: "df-btn is-ghost",
            onClick: clearSelectedEdgeFeedback
          }, t.feedbackMakeOrdinary)
        ) : import_react2.default.createElement(
          "div",
          { key: "ordinary", className: "df-advanced__content" },
          import_react2.default.createElement("span", { style: { color: "var(--df-ink-2)", fontSize: 12 } }, t.feedbackLoopNote),
          import_react2.default.createElement("button", {
            className: "df-btn",
        ...
        import_react2.default.createElement("button", { key: "del", className: "df-btn", style: { color: "var(--df-err)" }, onClick: removeSelectedEdge }, t.deleteEdge)
      ] : selectedNode ? [
  ...
  const assistantPanel = import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        "div",
        { className: "df-assist-menu-wrap", ref: assistMenuRef },
        import_react2.default.createElement(
          "button",
          {
            type: "button",
            ...
            ...(assistModelOptions ?? []).map(
              (option) => import_react2.default.createElement("button", {
                ...
                onClick: () => {
                  ...
                  setAssistProvider(option.provider ?? null);
                  setAssistMenuOpen(false);
                }
              }, option.model)
            )
  ...
  const workflowConfirmDialog = workflowOptimizeConfirm && import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        ...
        import_react2.default.createElement("button", { className: "df-btn", autoFocus: true, onClick: () => setWorkflowOptimizeConfirm(false) }, t.workflowOptimizeCancel),
  ...
  const cancelConfirmDialog = cancelConfirm && import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        "div",
        { className: "df-confirm__actions" },
        import_react2.default.createElement("button", { className: "df-btn", "data-df-action": "wait-cancel", autoFocus: true, onClick: () => setCancelConfirm(null) }, t.waitMore),
        import_react2.default.createElement("button", {
  ...
  const connectionWarningDialog = connectionWarning && import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        "div",
        { className: "df-confirm__actions" },
        import_react2.default.createElement("button", { className: "df-btn is-primary", autoFocus: true, onClick: () => setConnectionWarning(null) }, t.dismiss)
      )
    )
  );
  const deleteFlowConfirmDialog = deleteFlowConfirm && currentFlow && import_react2.default.createElement(
    "div",
    {
      className: "df-confirm-backdrop",
      role: "presentation",
    ...
    import_react2.default.createElement(
      "div",
      { className: "df-confirm", role: "alertdialog", "aria-modal": "true", "aria-labelledby": "df-delete-flow-title" },
      import_react2.default.createElement("h3", { id: "df-delete-flow-title" }, t.deleteFlowTitle + String(currentFlow.name ?? currentFlow.id)),
      import_react2.default.createElement("p", null, currentFlow.sessionId ? t.deleteFlowWarningOwned : t.deleteFlowWarningShared),
      import_react2.default.createElement(
        "div",
        { className: "df-confirm__actions" },
        import_react2.default.createElement("button", { className: "df-btn", autoFocus: true, disabled: deleteFlowBusy, onClick: () => setDeleteFlowConfirm(false) }, t.cancel),
        import_react2.default.createElement("button", {
          className: "df-btn is-primary",
  ...
  const switchFlowConfirmDialog = switchFlowTarget && import_react2.default.createElement(
    "div",
    {
      className: "df-confirm-backdrop",
      role: "presentation",
    ...
    import_react2.default.createElement(
      "div",
      { className: "df-confirm", role: "alertdialog", "aria-modal": "true", "aria-labelledby": "df-switch-flow-title" },
      import_react2.default.createElement("h3", { id: "df-switch-flow-title" }, t.switchFlowTitle),
      import_react2.default.createElement("p", null, t.switchFlowWarning),
      import_react2.default.createElement(
        "div",
        { className: "df-confirm__actions" },
        import_react2.default.createElement("button", { className: "df-btn", autoFocus: true, onClick: () => setSwitchFlowTarget(null) }, t.cancel),
        import_react2.default.createElement("button", {
          className: "df-btn is-primary",
  ...
  return import_react2.default.createElement(
    ...
    import_react2.default.createElement(
      ...
      import_react2.default.createElement(
        ...
        flows.length === 0 && import_react2.default.createElement(
          "div",
          { className: "df-empty-flow", "aria-live": "polite" },
          import_react2.default.createElement(
            "div",
            { className: "df-empty-flow__card" },
            import_react2.default.createElement("strong", null, allFlows.length > 0 ? t.noFlowWithHistory : t.noFlow),
            import_react2.default.createElement("span", null, allFlows.length > 0 ? t.emptyFlowHintWithHistory : t.emptyFlowHint)
          )
        ),
        topologyApplyButton
      ),

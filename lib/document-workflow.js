...
const UI_STRINGS = {
  en: {
    ...
    output: "Output",
    step: "Step",
    planLabel: "Plan & Decompose",
    implementLabel: "Implement",
    trialLabel: "Trial Run & Debug",
    qualityLabel: "Quality Check",
    planPrompt: "Understand the goal and constraints, check WORKFLOW.md, and produce an executable step-by-step plan.",
    implementPrompt: "Complete the core implementation according to the plan; save all artifacts in this step's workspace.",
    trialPrompt: "Launch and trial-run; take screenshots at key states, record issues, fixes, and re-test results.",
    qualityPrompt: "Run functional, build, regression, and UI quality checks; route back to fix when needed, then output an acceptance conclusion.",
    stepPrompt: (label) => `Complete "${label}" and pass the result to the next step.`
  },
  zh: {
    newFlow: "新工作流",
    input: "输入",
    ...
    step: "步骤",
    planLabel: "规划与拆解",
    implementLabel: "实现与产出",
    trialLabel: "试运行与截图调试",
    qualityLabel: "质量检查",
    planPrompt: "理解目标与约束，检查 WORKFLOW.md，并给出可执行的分步计划。",
    implementPrompt: "根据上一步计划完成核心实现，所有产物保存在本步骤工作区。",
    trialPrompt: "实际启动并试运行；在关键状态截图，记录问题、修复过程与复测结果。",
    qualityPrompt: "执行功能、构建、回归和界面质检；发现问题则退回对应步骤修复，确认通过后输出验收结论。",
    stepPrompt: (label) => `完成「${label}」，并把结果传递给下一步。`
  }
};
...
export function defaultWorkflowSteps(language = "zh") {
  const t = UI_STRINGS[language === "en" ? "en" : "zh"];
  return [
    { label: t.planLabel, prompt: t.planPrompt },
    { label: t.implementLabel, prompt: t.implementPrompt },
    { label: t.trialLabel, prompt: t.trialPrompt },
    { label: t.qualityLabel, prompt: t.qualityPrompt }
  ];
}

export function createScaffoldFlow(request = {}) {
  const now = new Date().toISOString();
  const language = request.language === "en" || request.locale === "en" ? "en" : "zh";
  const t = UI_STRINGS[language];
  const id = String(request.id ?? `flow-${Date.now().toString(36)}`);
  const rawSteps = Array.isArray(request.steps) && request.steps.length ? request.steps : defaultWorkflowSteps(language);
  const usedIds = new Set(["input", "output"]);
  const steps = rawSteps.map((value, index) => {
    const step = typeof value === "string"
      ? { label: value, prompt: t.stepPrompt(value) }
      : value ?? {};
    ...
    return {
      id: requestedId,
      kind,
      label: step.label ?? `${t.step} ${index + 1}`,
      content,
      data: step.data ?? {},
  ...
  let nodes = [
    { id: "input", kind: "input", data: { label: t.input } },
    ...steps.map((step) => ({
      id: step.id,
      kind: step.kind,
    ...
    { id: "output", kind: "output", data: { label: t.output } }
  ];
  ...
  return {
    id,
    version: 1,
    name: String(request.name ?? t.newFlow),

...
import { isFeedbackEdge } from "./graph-analysis.js";
...
export async function runAgentAssist(host, request) {
  ...
  try {
    ...
    // 模型优先级：用户在助手菜单的显式选择（可带 provider）> 插件配置 > 会话默认。
    // 只传模型名不传 provider 会落到会话默认 provider 上，跨 provider 同名模型会选错。
    const requestedModel = request.model || host.assistantModel;
    const modelOverride = (requestedModel || (selection?.provider && selection?.model))
      ? {
          provider: request.provider ?? selection?.provider ?? "deepseek-official",
          model: requestedModel ?? selection?.model,

...
import { optionalJsonArray, optionalJsonObject, requiredJsonObject } from "./tool-arguments.js";
...
/**
 * Agent 视角的激活工作流上下文：内存里记录每个 session 的 Agent 最近已知
 * activeFlowId；Studio 切换后第一次 flow_list/flow_read 返回 switched 通知，
 * 之后回到安静状态（工作流不变就不重复提醒，保持对话连续性）。
 */
async function agentFlowContext(host, sessionId) {
  if (!sessionId) return { activeFlowId: null, switched: null };
  const state = await host.store.session(sessionId);
  const pointer = typeof state.activeFlowId === "string" ? state.activeFlowId : null;
  // 指针指向的 flow 可能已被删除：只有仍可访问的 flow 才算激活。
  const activeFlowId = pointer && (await host.store.listFlow(sessionId, pointer)) ? pointer : null;
  // 用哨兵字符串区分「从未记录」与「已知为 null」：首次调用永不发通知
  //（还没有旧的工作流指令可忽略）；此后只要 activeFlowId 真的变了才发一次。
  const known = host.agentKnownActiveFlows.has(sessionId)
    ? host.agentKnownActiveFlows.get(sessionId)
    : "__unseen__";
  host.agentKnownActiveFlows.set(sessionId, activeFlowId);
  if (known === "__unseen__" || known === activeFlowId) return { activeFlowId, switched: null };
  return { activeFlowId, switched: { from: known, to: activeFlowId } };
}

/**
 * 记录「本 Session 最后一次使用的工作流」：Studio 重新打开/切回该 Session 时
 * 下拉自动选中它。仅创建/导入/画布选择时更新；只读的 flow_read 不抢指针，
 * 避免和用户在 Studio 里的手动切换互相打架。指针悬空时由 agentFlowContext
 * 与 dflow/activeFlow 自愈，无需显式清理。
 *
 * 这是保存后的「副作用」：任何失败都不能让已成功的主操作（flow_create /
 * flow_put / activate）报错——否则调用方会误以为保存失败而重试，产生重复工作流。
 */
async function setActiveFlowId(host, sessionId, flowId) {
  if (!sessionId) return;
  try {
    const state = await host.store.session(sessionId);
    if (state.activeFlowId === flowId) return;
    await host.store.updateSession(sessionId, (current) => ({ ...current, activeFlowId: flowId }));
  } catch (error) {
    console.error(`[deepseek-flow] active workflow pointer update failed: ${error?.message ?? error}`);
  }
}

function activeSwitchNotice(switched) {
  const from = switched.from === null ? "none" : switched.from;
  return [
    `The user switched the active workflow in Studio from "${from}" to "${switched.to}".`,
    "If the user asked to run a workflow earlier in this session, run the switched one (activeFlowId) and ignore those earlier instructions.",
    "If the user has not asked to run a workflow yet, continue the conversation naturally without mentioning the switch."
  ].join(" ");
}
...
async function saveSessionFlow(host, flow, sessionId, options = {}) {
  ...
  await host.store.updateSession(sessionId, async (state) => {
    ...
    // 跨 Session 导入（Studio 历史 dropdown / flow_put 搬运别的 session 的 flow）：
    // 源 docRoot 指向其他 session 的托管工作区时必须重定向到本 session 的新目录，
    // 否则两个 session 会写同一份 Markdown 互相覆盖。
    const imported = !previous && flow.sessionId && flow.sessionId !== sessionId
      ? (() => { const clone = { ...flow }; delete clone.docRoot; delete clone.sessionId; return clone; })()
      : flow;
    saved = {
      ...await prepareFlowWrite(host, imported, sessionId, previous, options),
      sessionId
    };
    return {
      ...state,
...
class DeepSeekFlowRemoteService extends TypertRemoteService {
  ...
  /** 当前 Session 的激活工作流指针（Studio 下拉选中项，跨重启持久化）。 */
  async activeFlow(sessionId) {
    if (!sessionId) return null;
    const state = await this.host.store.session(sessionId);
    const id = typeof state.activeFlowId === "string" ? state.activeFlowId : null;
    if (!id) return null;
    return (await this.host.store.listFlow(sessionId, id)) ? id : null;
  }
  /**
   * 跨 Session 历史工作流清单（轻量元数据，不读 Markdown 正文）。
   * Studio 下拉用它列出「历史的所有工作流」；数据源为磁盘存储，重启后仍完整。
   */
  async allFlows(sessionId) {
    const entries = await this.host.store.allFlows();
    const owned = new Set(sessionId ? (await this.host.store.session(sessionId)).flows.map((flow) => flow.id) : []);
    const active = await this.activeFlow(sessionId);
    return entries.map(({ flow, sessionId: ownerSessionId }) => ({
      id: flow.id,
      name: flow.name ?? flow.id,
      ownerSessionId: ownerSessionId ?? null,
      current: owned.has(flow.id),
  ...
  /**
   * 把另一个 Session / 共享模板的历史工作流切换为本 Session 的激活工作流。
   * - 已在当前 Session → 只更新指针，直接展示；
   * - 其他 Session 或共享模板 → 先以磁盘最新 Markdown 为准读取内容，
   *   再复制为当前 Session 的独立托管工作区副本（外部自定义目录原件不动，
   *   副本写入插件托管目录），互不污染；
   * - 每次切换都持久化 activeFlowId；空切换（activeFlowId=null）用于
   *   「本 Session 尚无激活工作流」的显式落盘。
   * 指针更新失败只记日志（setActiveFlowId 兜底），不会让已完成的复制报错。
   */
  async activate(request) {
    const { sessionId, flowId } = request ?? {};
    if (!sessionId) throw new Error("dflow/activate requires sessionId");
    if (flowId === undefined || flowId === null || flowId === "") {
      await setActiveFlowId(this.host, sessionId, null);
      return { activated: false, activeFlowId: null };
    }
    const lookup = await this.host.store.listFlow(sessionId, flowId);
    if (lookup) {
      await setActiveFlowId(this.host, sessionId, flowId);
      return { activated: true, activeFlowId: flowId, imported: false, flow: await this.loadDocs(lookup, sessionId) };
    }
    const [entry] = (await this.host.store.allFlows()).filter((candidate) => candidate.flow.id === flowId);
    if (!entry) throw new Error(`Flow ${flowId} was not found in any session or shared templates`);
    const source = entry.flow;
    // 先按源的原始归属读取磁盘最新 Markdown（共享模板用 shared scope），
    // 再剥离 docRoot/sessionId，让 saveSessionFlow 把副本落到本 Session
    // 的独立托管工作区：跨 Session / 共享模板都不会互相污染。
    const sourceScope = entry.sessionId ?? "shared";
    const documented = await this.loadDocs(source, sourceScope);
    const imported = { ...documented, sessionId: sourceScope };
    delete imported.docRoot;
    const result = await saveSessionFlow(this.host, imported, sessionId, { documentPolicy: "prefer-flow" });
    await setActiveFlowId(this.host, sessionId, flowId);
    return {
      activated: true,
      activeFlowId: flowId,
...
export function registerTools(ctx, host) {
  ...
  ctx.tools.register(defineTool({
    name: "flow_create",
    description: "优先用这个工具新建 DeepSeekFlow 工作流。默认生成线性链；传 connections 后可生成安全的分支图。普通执行箭头必须无环；有限重试可在连接上添加 feedback={maxIterations,exitCondition}。逻辑门不是自然语言判断器：condition 的 data.gateType 只能是 ifElse/and/or/not/nand/nor/xor/xnor，data.predicate 只能是 truthy/falsy/nonEmpty。需要理解中文条件时，先放一个 Agent 输出 Boolean true/false，再连到 predicate=truthy 的 condition。系统会直接持久化，无需再让 Studio 交回主 Session 审核。",
    parameters: {
      ...
      connections: { type: "json", description: "可选连线数组 [{source,target,branch?,feedback?}]。IF/ELSE 出边 branch 分别写 true、false；其他门写对应门名。普通箭头必须无环；有限回边写 feedback:{maxIterations:3,exitCondition:\"通过质检\"}，且不能带 branch。" },
      doc_root: { type: "string", description: "可选的文档工作区绝对路径；省略时保存到插件工作区。" },
      language: { type: "string", description: "默认标签语言：en 或 zh。省略时按用户语言；Agent 调用时应使用用户当前交流语言。" }
    },
    output: { schema: { type: "json" }, render: renderJson },
    async execute(args, exec) {
      const sessionId = sessionOf(exec);
      if (!sessionId) throw new Error("No current session");
      const steps = optionalJsonArray(args, "steps");
      const connections = optionalJsonArray(args, "connections");
      const language = args.language === "en" ? "en" : "zh";
      ...
      await setActiveFlowId(host, sessionId, next.id);
      return {
        ok: true,
        id: next.id,
        name: next.name,
  ...
  ctx.tools.register(defineTool({
    name: "flow_list",
    description: "列出当前 Session 可用的 DeepSeekFlow 定义（当前 Session 的 + 共享模板），并标记当前激活的工作流（activeFlowId = Studio 下拉中最后选中/使用的）。若返回了 activeFlowNotice，说明用户刚在 Studio 切换了激活工作流：请立即改用 activeFlowId 指向的工作流，忽略会话早期关于运行其他工作流的指令；没有 activeFlowNotice 就保持对话连续性，不要重提切换。DeepSeekFlow 负责编辑与布尔门求值，但不运行 Agent 步骤；选定 id 后用 flow_read 读取文档，并在当前 Session 内按步骤执行。",
    parameters: {},
    output: { schema: { type: "json" }, render: renderJson },
    async execute(_args, exec) {
      ...
      const ownedIds = new Set(owned.map((flow) => flow.id));
      const context = await agentFlowContext(host, sessionId);
      return {
        currentSessionId: sessionId ?? null,
        activeFlowId: context.activeFlowId,
        ...(context.switched ? { activeFlowNotice: activeSwitchNotice(context.switched) } : {}),
        flows: [...owned, ...shared.filter((flow) => !ownedIds.has(flow.id))].map((f) => ({
          id: f.id,
          name: f.name,
          sessionId: f.sessionId ?? null,
          active: f.id === context.activeFlowId,
          ...
          return {
            ...
            settings: Object.fromEntries(Object.entries({
              ...
              inputPredicates: node.data?.inputPredicates,
                  gateType: node.kind === "condition" ? conditionGateType(node, flow.edges.filter((edge) => edge.source === node.id && edge.feedback === undefined)) : undefined,
            ...
            edges: flow.edges.map((edge) => ({
              ...
              ...(edge.feedback === undefined && gateBranchForEdge(edge) !== undefined ? { branch: gateBranchForEdge(edge) } : {})
            }))
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const stored = await host.store.listFlow(sessionId, args.id);
      if (!stored) throw new Error(`Flow ${args.id} was not found`);
      const evaluation = evaluateFlowLogic(stored, requiredJsonObject(args, "values"));
      return {
        id: stored.id,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const sessionId = sessionOf(exec);
      let flow = optionalJsonObject(args, "flow");
      if (flow === undefined) {
...
export function isMissingLegacyStateError(error) {
  return error?.code === "ENOENT"
    && /harness-flow[\\/]+state\.json/.test(String(error?.path ?? error?.message ?? ""));
}

export async function apply(ctx, config) {
  // 运行时自愈：如果 ctx.tools 的 TOOL_RUNTIME_SCHEDULER 因 dsh-tools 模块副本而缺失，
  // 用 Symbol.for 全局注册表桥接到宿主的 scheduler。（ref: issue #1 「修法 1」）
  if (ctx.tools && !ctx.tools[TOOL_RUNTIME_SCHEDULER]) {
    const universalKey = Symbol.for("@deepseek-ai/dsh-tools.scheduler");
    const scheduler = ctx.tools[universalKey];
    if (scheduler) {
      Object.defineProperty(ctx.tools, TOOL_RUNTIME_SCHEDULER, {
        value: scheduler, writable: false, configurable: true
      });
      console.log("[deepseek-flow] self-healed TOOL_RUNTIME_SCHEDULER mismatch");
    }
  }
  ...
  if (!bundledSkillContent) throw new Error("Bundled deepseek-flow SKILL.md has no Markdown body after frontmatter");
    ...
    skillCtx.skills.register({
      name: "deepseek-flow",
      description: "把用户的工作流需求落地为 DeepSeek Flow 可视化流程图：用户直说「构建/导入工作流、把流程做成图」时；或用户描述一个多步骤任务流程（如「做一个找论文、下论文、读论文、出综述和名词解释的工作流」）时，识别需求、拆解步骤，用 flow_create 生成总控 WORKFLOW.md + 每步 STEP.md 独立工作区 + 画布节点连线；或用 flow_put 导入既有 flow.json。运行工作流前先 flow_list 检查 activeFlowNotice：用户在 Studio 切换过激活工作流时改用 activeFlowId 并忽略早期运行指令，未切换则保持对话连续性。",
      whenToUse: "用户要求构建/建立/生成/导入工作流、把流程做成图、可视化流程、或要求运行某个工作流时使用；用户描述了一个多步骤的任务流程需求（找论文→读论文→出综述→名词解释 这类「先…再…最后…」的流程）时也使用。",
  ...
  try {
    ...
    if (isMissingLegacyStateError(error)) {
      console.log("[deepseek-flow] no legacy harness-flow state to migrate");
    } else {
...
export { agentFlowContext, setActiveFlowId, activeSwitchNotice };

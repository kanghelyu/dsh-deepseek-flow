...
function validateFlow(flow) {
  ...
  for (const edge of flow.edges) {
    ...
    if (edge.sourceHandle !== undefined && source?.kind !== "condition") issues.push(`branch handle on non-condition edge ${edge.id}`);
    if (source?.kind === "condition" && edge.sourceHandle === undefined) issues.push(`condition edge ${edge.id} requires true or false handle`);
  }
    ...
    async execute(args, exec) {
      ...
      return {
        ...
        edges: flow.edges.map((edge) => ({
          source: edge.source,
          target: edge.target,
          ...(edge.sourceHandle === undefined ? {} : { branch: edge.sourceHandle })
        }))
      };
    }
  }));

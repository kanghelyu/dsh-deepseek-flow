...
export function buildTopologyReviewPrompt(baseFlow, draftFlow, staticIssues = []) {
  ...
  return [
    ...
    "Each node may contain only id, kind, position, data; data only label, gateType, predicate, inputPredicates, order. Each edge only id, source, target, sourceHandle, targetHandle, label.",

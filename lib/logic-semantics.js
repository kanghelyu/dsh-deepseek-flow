...
function conditionInputs(flow, node) {
  return flow.edges
    .filter((edge) => edge.target === node.id && !isFeedbackEdge(edge))
    .map((edge) => ({
      edgeId: edge.id,
      source: edge.source,
...
export function logicExecutionContract(flow) {
  ...
  return {
    ...
    feedbackLoops: feedbackEdges(flow?.edges).map((edge) => ({
      edgeId: edge.id,
      source: edge.source,
      target: edge.target,
...
export function evaluateFlowLogic(flow, values = {}) {
  ...
  if (!analysis.complete) {
    throw new LogicEvaluationError(["cycle detected in ordinary execution edges; Boolean propagation ignores only explicit feedback edges"]);
  }
